...
import {
  ...
  FileText, MessageCircle, Zap, Search,
} from 'lucide-react'
import { useAuth } from '../context/AuthContext'
import { trainerRequestService, trainerService, userService } from '../services/firestore'
...
export default function TrainerPage() {
  ...
  const loadRequests = async () => {
    setLoading(true)
    try {
      const reqs = await trainerRequestService.getPending()
      setRequests(reqs)
  ...
  const handleComplete = async (requestId) => {
    // For now, mark as complete. In a full flow, trainer would build/edit the workout first.
    try {
      await trainerRequestService.complete(requestId, null, '')
      await loadRequests()
    } catch (err) {
    ...
    <div className="max-w-4xl mx-auto pb-24">
      ...
      <div className="flex gap-2 mb-6">
        {['active', 'all'].map(f => (
          ...
          >
            {f === 'active' ? 'Active Requests' : 'All Requests'}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="card-steel p-12 text-center">
          ...
          <h3 className="text-lg font-medium text-iron-200 mb-1">All caught up</h3>
          <p className="text-iron-500">No pending requests right now.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {requests
            .filter(r => filter === 'active' ? ['pending', 'in_progress'].includes(r.status) : true)
                          ...
                          {req.type === 'review' && req.workoutId && (
                            <button
                              ...
                              className="mt-4 w-full p-3 bg-purple-500/10 border border-purple-500/20 rounded-lg
                                text-sm text-purple-300 hover:bg-purple-500/20 transition-colors
                                flex items-center gap-2"
                            >
                              <FileText className="w-4 h-4" />
                              Open Workout to Review & Edit
                            </button>
                          )}

                          {/* User Summary */}
                          <div className="mt-2">
                            <h4 className="text-xs font-medium text-iron-400 uppercase tracking-wider mb-3">
                              Athlete Summary
                            </h4>
                            {isSummaryLoading ? (
                                ...
                                {Object.keys(summary.maxLifts || {}).length > 0 && (
                                  <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-1.5">
                                      {Object.entries(summary.maxLifts)
                            ...
                            {req.status === 'in_progress' && req.trainerId === user?.uid && (
                              <>
                                ...
                                <button
                                  ...
                                  className="flex-1 py-2.5 bg-green-500/20 hover:bg-green-500/30 text-green-400 rounded-lg
                                    text-sm font-medium flex items-center justify-center gap-2 transition-colors"
                                >
